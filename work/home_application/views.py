...
def host_data(request):
    if request.method == 'POST':
        input1 = request.POST.get('zhuti',None)
        input2 = request.POST.get('neirong',None)

        if "" in [input1,input2]:
            return  render_json({'code': -1,'msg':'some of params lost.'})

        try:
            HostModel.object.create(zhuti = input1,neirong = input2)
        except Exception as e:
             return  render_json({'code': -1,'msg':'something wrong.'})
    else:
